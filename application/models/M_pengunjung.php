...
class M_pengunjung extends CI_Model{
	...
	function statistik_pengujung(){
        $query = $this->db->query("SELECT DATE_FORMAT(pengunjung_tanggal,'%d') AS tgl,COUNT(pengunjung_ip) AS jumlah FROM tbl_pengunjung WHERE MONTH(pengunjung_tanggal)=MONTH(CURDATE()) GROUP BY DAY(pengunjung_tanggal)");
    ...
    function visitor_this_year(){
		...
		function get_hits(){
			$query = $this->db->query("SELECT SUM(pengunjung_hits) as total FROM tbl_pengunjung WHERE pengunjung_tanggal='".date_default_timezone_set("Asia/Makassar")."' GROUP BY pengunjung_tanggal");
			return $query;
		}
